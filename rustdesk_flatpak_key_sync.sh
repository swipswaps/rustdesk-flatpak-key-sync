#!/usr/bin/env bash
# ==============================================================================
# rustdesk_flatpak_key_sync.sh (v5.0)
# ------------------------------------------------------------------------------
# Author: Gemini
# Date: 2025-11-08
#
# Auto-syncs RustDesk server public key to LAN clients, supporting both
# standard and Flatpak installations. Features include automatic dependency
# installation, robust SSH context handling under sudo, password-based SSH
# key bootstrapping, and an interactive host selection menu.
#
# ==============================================================================
...
# User on the client machines
readonly CLIENT_USER="owner"
# LAN subnet to scan
...
check_dependencies() {
    local missing_deps=()
    for cmd in ssh scp sshpass nmap awk grep sed; do
...
prepare_key() {
    ...
    if [[ ! -f "${PUB}" ]]; then
        if [[ -f "${PRIV}" ]]; then
            log "Public key not found, generating from private key..."
            ssh-keygen -y -f "${PRIV}" > "${TMP_PUB}"
        else
            fail "RustDesk private key not found at ${PRIV}. Cannot generate public key."
        fi
    else
...
discover_hosts() {
    ...
    nmap -p 22 --open "${LAN_SUBNET}" -oG - | awk '/Up$/{print $2}'
}
...
sync_to_host() {
    ...
    local remote_config_dir_std="/home/${user}/.config/RustDesk"
    local remote_config_dir_flatpak="/home/${user}/.var/app/com.rustdesk.RustDesk/config/RustDesk"
    local remote_pub_key_path_std="${remote_config_dir_std}/rustdesk.pub"
    local remote_pub_key_path_flatpak="${remote_config_dir_flatpak}/rustdesk.pub"
    local config_dir=""
    local pub_key_path=""

    log "Syncing to ${host}..."

    # Determine if standard or flatpak config exists
    if sshpass -p "$pass" ssh -o StrictHostKeyChecking=no "${user}@${host}" "test -d ${remote_config_dir_flatpak}"; then
        config_dir="${remote_config_dir_flatpak}"
        pub_key_path="${remote_pub_key_path_flatpak}"
        log "Flatpak installation detected on ${host}."
    elif sshpass -p "$pass" ssh -o StrictHostKeyChecking=no "${user}@${host}" "test -d ${remote_config_dir_std}"; then
        config_dir="${remote_config_dir_std}"
        pub_key_path="${remote_pub_key_path_std}"
        log "Standard installation detected on ${host}."
    else
        log "No RustDesk configuration directory found on ${host}. Creating standard config directory."
        sshpass -p "$pass" ssh -o StrictHostKeyChecking=no "${user}@${host}" "mkdir -p ${remote_config_dir_std}"
        config_dir="${remote_config_dir_std}"
        pub_key_path="${remote_pub_key_path_std}"
    fi

    # Check for existing key and prompt for overwrite
    if sshpass -p "$pass" ssh -o StrictHostKeyChecking=no "${user}@${host}" "test -f ${pub_key_path}"; then
    ...
    if sshpass -p "$pass" scp -o StrictHostKeyChecking=no "${TMP_PUB}" "${user}@${host}:${pub_key_path}"; then
        echo "Synced"
    else
...
main() {
    ...
    log "===== RustDesk Flatpak Key Sync Initialized ====="
