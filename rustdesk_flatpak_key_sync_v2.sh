...
auto_install_deps() {
  ...
  install_if_missing "firewalld" "firewalld" "$pkgmgr" || true
  pass "Dependencies verified/installed (or skipped for unknown distro)"
}

# ------------------------------ Avahi/mDNS Setup -------------------------------
ensure_avahi() {
  log "Ensuring Avahi daemon is running for mDNS..."
  ...
  if require_cmd firewall-cmd; then
    ...
    pass "Avahi/mDNS + firewall configured"
  fi
}
...
remote_restart_flatpak() {
  ...
  ssh -o ConnectTimeout="$SSH_TIMEOUT" "$CLIENT_USER@$host" \
    "flatpak kill com.rustdesk.RustDesk || true; nohup flatpak run -d com.rustdesk.RustDesk >/dev/null 2>&1 &" &>>"$LOG_FILE" || warn "Cannot restart RustDesk on $host"
}
...
for host in "${HOSTS[@]}"; do
  ...
  [[ $verified -eq 0 ]] && warn "Verification failed on $host"
done
